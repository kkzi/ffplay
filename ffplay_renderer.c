...
static int add_instance_extension(const char **ext, unsigned num_ext, const AVDictionary *opt, AVDictionary **dict)
{
...
static int add_device_extension(const AVDictionary *opt, AVDictionary **dict)
{
...
static int create_vk_by_hwcontext(VkRenderer *renderer, const char **ext, unsigned num_ext, const AVDictionary *opt)
{
    ...
    ret = av_hwdevice_ctx_create(&ctx->hw_device_ref, AV_HWDEVICE_TYPE_VULKAN, select_device(opt), dict, 0);
    av_dict_free(&dict);
    if (ret < 0)
        return ret;
...
static void placebo_lock_queue(struct AVHWDeviceContext *dev_ctx, uint32_t queue_family, uint32_t index)
{
...
static void placebo_unlock_queue(struct AVHWDeviceContext *dev_ctx, uint32_t queue_family, uint32_t index)
{
    RendererContext *ctx = dev_ctx->user_opaque;
    pl_vulkan vk = ctx->placebo_vulkan;
...
static int get_decode_queue(VkRenderer *renderer, int *index, int *count)
{
    ...
    *index = -1;
    *count = 0;
    get_queue_family_prop =
        (PFN_vkGetPhysicalDeviceQueueFamilyProperties)get_proc_addr(ctx->placebo_instance->instance, "vkGetPhysicalDeviceQueueFamilyProperties");
    get_queue_family_prop(ctx->placebo_vulkan->phys_device, &num_queue_family_prop, NULL);
    ...
    get_queue_family_prop(ctx->placebo_vulkan->phys_device, &num_queue_family_prop, queue_family_prop);
...
static int create_vk_by_placebo(VkRenderer *renderer, const char **ext, unsigned num_ext, const AVDictionary *opt)
{
    RendererContext *ctx = (RendererContext *)renderer;
    ...
    ctx->placebo_instance = pl_vk_inst_create(
        ctx->vk_log, pl_vk_inst_params(.get_proc_addr = ctx->get_proc_addr, .debug = enable_debug(opt), .extensions = ext, .num_extensions = num_ext));
    ...
    ctx->placebo_vulkan = pl_vulkan_create(ctx->vk_log,
        pl_vulkan_params(.instance = ctx->placebo_instance->instance, .get_proc_addr = ctx->placebo_instance->get_proc_addr, .surface = ctx->vk_surface,
            .allow_software = false, .opt_extensions = optional_device_exts, .num_opt_extensions = FF_ARRAY_ELEMS(optional_device_exts),
            .extra_queues = VK_QUEUE_VIDEO_DECODE_BIT_KHR, .device_name = select_device(opt), ));
    if (!ctx->placebo_vulkan)
        return AVERROR_EXTERNAL;
    ...
    vk_dev_ctx->lock_queue = placebo_lock_queue, vk_dev_ctx->unlock_queue = placebo_unlock_queue;
...
static int create(VkRenderer *renderer, SDL_Window *window, AVDictionary *opt)
{
    ...
    ctx->swapchain =
        pl_vulkan_create_swapchain(ctx->placebo_vulkan, pl_vulkan_swapchain_params(.surface = ctx->vk_surface, .present_mode = VK_PRESENT_MODE_FIFO_KHR));
    if (!ctx->swapchain)
    {
        ret = AVERROR_EXTERNAL;
        goto out;
...
static int create_hw_frame(VkRenderer *renderer, AVFrame *frame)
{
    ...
    if (ctx->hw_frame_ref)
    {
        ...
        if (hw_frame->width == frame->width && hw_frame->height == frame->height && hw_frame->sw_format == src_hw_frame->sw_format)
            return 0;
    ...
    av_hwframe_transfer_get_formats(ctx->hw_frame_ref, AV_HWFRAME_TRANSFER_DIRECTION_TO, &ctx->transfer_formats, 0);
...
static void destroy(VkRenderer *renderer)
{
    ...
    if (ctx->vk_surface)
    {
        vkDestroySurfaceKHR = (PFN_vkDestroySurfaceKHR)ctx->get_proc_addr(ctx->inst, "vkDestroySurfaceKHR");
