...
static int64_t frame_end(const SyncQueue *sq, SyncQueueFrame frame, int nb_samples)
{
    if (nb_samples)
    {
        int64_t d = av_rescale_q(nb_samples, (AVRational){ 1, frame.f->sample_rate }, frame.f->time_base);
        return frame.f->pts + d;
    }

    return (sq->type == SYNC_QUEUE_PACKETS) ? frame.p->pts + frame.p->duration : frame.f->pts + frame.f->duration;
}
...
{
    AVRational tb = (sq->type == SYNC_QUEUE_PACKETS) ? frame.p->time_base : frame.f->time_base;
...
static void finish_stream(SyncQueue *sq, unsigned int stream_idx)
{
    ...
    if (st->limiting && st->head_ts != AV_NOPTS_VALUE)
    {
        ...
        if (sq->head_finished_stream < 0 ||
            av_compare_ts(st->head_ts, st->tb, sq->streams[sq->head_finished_stream].head_ts, sq->streams[sq->head_finished_stream].tb) < 0)
        {
        ...
        for (unsigned int i = 0; i < sq->nb_streams; i++)
        {
            ...
            if (st != st1 && st1->head_ts != AV_NOPTS_VALUE && av_compare_ts(st->head_ts, st->tb, st1->head_ts, st1->tb) <= 0)
            {
                if (!st1->finished)
...
static void queue_head_update(SyncQueue *sq)
{
    ...
    for (unsigned int i = 0; i < sq->nb_streams; i++)
    {
        ...
        if (st_other->limiting && st_other->head_ts != AV_NOPTS_VALUE && av_compare_ts(st_other->head_ts, st_other->tb, st_head->head_ts, st_head->tb) < 0)
            sq->head_stream = i;
    }
}
...
static void stream_update_ts(SyncQueue *sq, unsigned int stream_idx, int64_t ts)
{
    ...
    if (ts == AV_NOPTS_VALUE || (st->head_ts != AV_NOPTS_VALUE && st->head_ts >= ts))
        return;
    ...
    if (sq->head_finished_stream >= 0 &&
        av_compare_ts(sq->streams[sq->head_finished_stream].head_ts, sq->streams[sq->head_finished_stream].tb, ts, st->tb) <= 0)
...
static int overflow_heartbeat(SyncQueue *sq, int stream_idx)
{
    ...
    if (stream_idx < 0)
    {
        ...
        {
            st = &sq->streams[i];
            if (st->head_ts != AV_NOPTS_VALUE && (ts == AV_NOPTS_VALUE || av_compare_ts(ts, sq->streams[stream_idx].tb, st->head_ts, st->tb) < 0))
            {
                ts = st->head_ts;
                stream_idx = i;
            }
    ...
    for (size_t i = 0; tail_ts == AV_NOPTS_VALUE && av_fifo_peek(st->fifo, &frame, 1, i) >= 0; i++)
        tail_ts = frame_end(sq, frame, 0);

    /* overflow triggers when the tail is over specified duration behind the head */
    if (tail_ts == AV_NOPTS_VALUE || tail_ts >= st->head_ts || av_rescale_q(st->head_ts - tail_ts, st->tb, AV_TIME_BASE_Q) < sq->buf_size_us)
        return 0;
    ...
    for (unsigned int i = 0; i < sq->nb_streams; i++)
    {
        ...
        if (st == st1 || st1->finished || (st1->head_ts != AV_NOPTS_VALUE && av_compare_ts(tail_ts, st->tb, st1->head_ts, st1->tb) <= 0))
            continue;
...
int sq_send(SyncQueue *sq, unsigned int stream_idx, SyncQueueFrame frame)
{
    ...
    if (nb_samples)
    {
        ...
        dst.f->duration = av_rescale_q(nb_samples, (AVRational){ 1, dst.f->sample_rate }, dst.f->time_base);
    }

    ts = frame_end(sq, dst, 0);

    av_log(sq->logctx, AV_LOG_DEBUG, "sq: send %u ts %s\n", stream_idx, av_ts2timestr(ts, &st->tb));
...
static void offset_audio(AVFrame *f, int nb_samples)
{
    ...
    f->duration = av_rescale_q(f->nb_samples, (AVRational){ 1, f->sample_rate }, f->time_base);
    f->pts += av_rescale_q(nb_samples, (AVRational){ 1, f->sample_rate }, f->time_base);
}
...
static int frame_is_aligned(const SyncQueue *sq, const AVFrame *frame)
{
    ...
    if (!((uintptr_t)frame->data[0] & sq->align_mask) && !(frame->linesize[0] & sq->align_mask) && frame->linesize[0] > sq->align_mask)
        return 1;
...
static int receive_samples(SyncQueue *sq, SyncQueueStream *st, AVFrame *dst, int nb_samples)
{
    ...
    while (dst->nb_samples < nb_samples)
    {
        ...
        av_samples_copy(dst->extended_data, src.f->extended_data, dst->nb_samples, 0, to_copy, dst->ch_layout.nb_channels, dst->format);
...
finish:
    dst->duration = av_rescale_q(nb_samples, (AVRational){ 1, dst->sample_rate }, dst->time_base);
...
{
    const SyncQueueStream *st_head = sq->head_stream >= 0 ? &sq->streams[sq->head_stream] : NULL;
    SyncQueueStream *st;

    av_assert0(stream_idx < sq->nb_streams);
    st = &sq->streams[stream_idx];

    if (av_fifo_can_read(st->fifo) && (st->frame_samples <= st->samples_queued || st->finished))
    {
        ...
        {
            if (nb_samples && (nb_samples != peek.f->nb_samples || !frame_is_aligned(sq, peek.f)))
            {
            ...
            av_log(sq->logctx, AV_LOG_DEBUG, "sq: receive %u ts %s queue head %d ts %s\n", stream_idx, av_ts2timestr(frame_end(sq, frame, 0), &st->tb),
                sq->head_stream, st_head ? av_ts2timestr(st_head->head_ts, &st_head->tb) : "N/A");

            return 0;
        }
    }

    return (sq->finished || (st->finished && !av_fifo_can_read(st->fifo))) ? AVERROR_EOF : AVERROR(EAGAIN);
}
...
SyncQueue *sq_alloc(enum SyncQueueType type, int64_t buf_size_us, void *logctx)
{
    ...
    sq->pool = (type == SYNC_QUEUE_PACKETS) ? objpool_alloc_packets() : objpool_alloc_frames();
    if (!sq->pool)
    {
        av_freep(&sq);
        return NULL;
