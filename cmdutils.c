...
static void prepare_app_arguments(int *argc_ptr, char ***argv_ptr)
{
    ...
    for (i = 0; i < win32_argc; i++)
    {
        ...
        offset += WideCharToMultiByte(CP_UTF8, 0, argv_w[i], -1, &argstr_flat[offset], buffsize - offset, NULL, NULL);
    }
...
static int write_option(void *optctx, const OptionDef *po, const char *opt, const char *arg, const OptionDef *defs)
{
     ...
     * a global var*/
    void *dst = po->flags & OPT_FLAG_OFFSET ? (uint8_t *)optctx + po->u.off : po->u.dst_ptr;
    ...
    if (*opt == '/')
    {
        ...
        {
            av_log(NULL, AV_LOG_FATAL, "Requested to load an argument from file for a bool option '%s'\n", po->name);
            return AVERROR(EINVAL);
        }
        ...
        if (!arg_allocated)
        {
            av_log(NULL, AV_LOG_FATAL, "Error reading the value for option '%s' from file: %s\n", opt, arg);
            return AVERROR(EINVAL);
        }
    ...
    {
        sol->type = po->type;
        sol->opt_canon = (po->flags & OPT_HAS_CANON) ? find_option(defs, po->u1.name_canon) : po;
    }
...
int opt_default(void *optctx, const char *opt, const char *arg)
{
    ...
    if ((o = opt_find(&cc, opt_stripped, NULL, 0, AV_OPT_SEARCH_CHILDREN | AV_OPT_SEARCH_FAKE_OBJ)) ||
        ((opt[0] == 'v' || opt[0] == 'a' || opt[0] == 's') && (o = opt_find(&cc, opt + 1, NULL, 0, AV_OPT_SEARCH_FAKE_OBJ))))
    {
    ...
    {
        if (!strcmp(opt, "srcw") || !strcmp(opt, "srch") || !strcmp(opt, "dstw") || !strcmp(opt, "dsth") || !strcmp(opt, "src_format") ||
    ...
    else
    {
        ...
        if (datadir)
        {
            ...
            if (ls = strrchr(datadir, '/'))
            {
                ...
                char *new_datadir = av_realloc_array(datadir, desired_size, sizeof *datadir);
                if (new_datadir)
                {
                    datadir = new_datadir;
...
void *allocate_array_elem(void *ptr, size_t elem_size, int *nb_elems)
{
    ...
    if (!(new_elem = av_mallocz(elem_size)) || av_dynarray_add_nofree(ptr, nb_elems, new_elem) < 0)
        return NULL;
    return new_elem;
}
